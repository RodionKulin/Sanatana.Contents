    ...
    public static class HtmlParser
    {
        ...
        private static int FindClosingTagIndex(string tagName, string html, int startIndex)
        {
            ...
            while (ix > -1 && ix < html.Length)
            {
                ...
                int ix1 = html.IndexOf(tag1, ix, StringComparison.OrdinalIgnoreCase);
                int ix2 = html.IndexOf(tag2, ix, StringComparison.OrdinalIgnoreCase);
                ...
                int ix3 = html.IndexOf("<script ", ix, StringComparison.OrdinalIgnoreCase);
                if (ix3 > -1 && ix3 < ix2)
                {
                    ix3 = html.IndexOf("</script>", ix3 + 7, StringComparison.OrdinalIgnoreCase);
                    if (ix3 > ix2)
                    {
                        ix = ix3 + 8;
                        continue;
